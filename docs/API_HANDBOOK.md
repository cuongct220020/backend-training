...
### 2.4. Token-based Authentication & JSON Web Tokens (JWT)
...
### 2.5. OAuth 2.0 - Giao thức phân quyền
...
### 2.7. Kết luận về cơ chế xác thực
...
| Tính năng | Xác thực dựa trên Session | Xác thực dựa trên Token (JWT) |
|-----------|---------------------------|-------------------------------|
| **Quản lý Trạng thái** | Stateful (Có trạng thái): Server lưu trữ bản ghi session cho mỗi người dùng đang hoạt động. | Stateless (Phi trạng thái): Server không lưu trữ trạng thái. Mọi thông tin cần thiết đều nằm trong token. |
| **Khả năng mở rộng** | Kém: Khó mở rộng theo chiều ngang. Yêu cầu "sticky sessions" hoặc kho lưu trữ session tập trung. | Tốt: Dễ dàng mở rộng vì không có trạng thái session cần đồng bộ hóa giữa các server. |
| **Hiệu suất** | Chậm hơn: Yêu cầu một lượt tra cứu (lookup) trong cơ sở dữ liệu/bộ nhớ đệm trên mỗi yêu cầu. | Nhanh hơn: Chỉ cần xác minh chữ ký bằng CPU, không cần tra cứu I/O. |
| **Bảo mật (CSRF)** | Dễ bị tấn công: Phụ thuộc vào cookie, vốn dễ bị tấn công CSRF nếu không có các biện pháp phòng chống (SameSite, CSRF token). | Ít bị ảnh hưởng hơn: Không phụ thuộc vào cookie tự động gửi đi, nhưng vẫn cần phòng chống nếu token được lưu trong cookie. |
| **Bảo mật (XSS/Trộm cắp)** | An toàn hơn (nếu dùng HttpOnly): Cookie `HttpOnly` không thể bị truy cập bởi JavaScript, giúp chống lại việc đánh cắp session qua XSS. | Dễ bị tấn công: Nếu token được lưu trong `localStorage`, nó có thể bị đánh cắp qua tấn công XSS. |
| **Kiểm soát/Thu hồi** | Dễ dàng: Server có thể vô hiệu hóa một session ngay lập tức bằng cách xóa bản ghi của nó. | Khó khăn: Một token đã cấp không thể bị thu hồi. Nó hợp lệ cho đến khi hết hạn. Cần các giải pháp phức tạp như danh sách đen. |
| **Tương thích Đa miền/Microservices** | Khó khăn: Cookie bị giới hạn bởi chính sách cùng nguồn gốc (same-origin policy), gây khó khăn cho việc chia sẻ session giữa các domain hoặc dịch vụ khác nhau. | Lý tưởng: Token có thể dễ dàng được gửi đến bất kỳ domain hoặc microservice nào, miễn là dịch vụ đó có khóa để xác minh chữ ký. |
| **Trường hợp sử dụng điển hình** | Các ứng dụng web truyền thống, nguyên khối (monolithic) nơi khả năng thu hồi tức thì là quan trọng. | Các ứng dụng trang đơn (SPA), ứng dụng di động, kiến trúc microservices, các API được sử dụng bởi bên thứ ba. |


## 3. Nền tảng bảo mật Web và API theo chuẩn OWASP
...
### 3.2. Phân tích chi tiết OWASP API Security Top 10 (phiên bản 2023)
...
#### 3.2.1. API1:2023 - Broken Object Level Authorization (BOLA)
...
#### 3.2.2. API2:2023 - Broken Authentication
...
#### 3.2.3. API3:2023 - Broken Object Property Level Authorization
...
#### 3.2.4. API4:2023 - Unrestricted Resource Consumption
...
#### 3.2.5. API5:2023 - Broken Function Level Authorization (BFLA)
...
#### 3.2.6. API6:2023 - Unrestricted Access to Sensitive Business Flows
...
#### 3.2.7. API7:2023 - Server Side Request Forgery (SSRF)
...
#### 3.2.8. API8:2023 - Security Misconfiguration
...
#### 3.2.9. API9:2023 - Improper Inventory Management
...
#### 3.2.10. API10:2023 - Unsafe Consumption of APIs
...
### 3.3. Nhận xét về sự thay đổi trong OWASP API Security Top 10
...
### 3.4. Áp dụng OWASP Web Checklist vào quy trình phát triển
...
## 4. Các hàm băm (Hashing Functions) và lưu trữ mật khẩu an toàn
...
### 4.1. Nguyên tắc cơ bản của Hashing
...
### 4.2. Tại sao MD5 và SHA-1 không còn an toàn
...
### 4.3. Phân tích các thuật toán hiện đại: bcrypt, scrypt và Argon2
...
### 4.4. Sự tiến hóa của thuật toán băm mật khẩu
...
| Tính năng | bcrypt | scrypt | Argon2 |
|-----------|--------|--------|--------|
| **Năm giới thiệu** | 1999 | 2009 | 2015 |
| **Kháng cự chính** | CPU-hard (Giới hạn bởi CPU) | Memory-hard (Giới hạn bởi bộ nhớ) | Memory-hard & CPU-hard |
| **Giới hạn bởi bộ nhớ** | Không | Có | Có (Tùy chỉnh được) |
| **Song song hóa** | Không | Có | Có (Tùy chỉnh được) |
| **Kháng tấn công kênh phụ** | Một phần | Không | Có (với Argon2i/id) |
| **Khả năng cấu hình** | Thấp (chỉ cost factor) | Trung bình | Cao (bộ nhớ, thời gian, song song) |
| **Trạng thái khuyến nghị** | Chấp nhận được, đã được kiểm chứng | Tốt, kháng GPU tốt | Khuyến nghị hàng đầu (Argon2id) |
...
## 5. Tổng hợp các API Security Best Practices
...
### 5.1. Bảo mật đường truyền với SSL/TLS
...
### 5.4. Cứng hóa máy chủ và ứng dụng
...
### 5.5. Checklist Bảo mật API Toàn diện
